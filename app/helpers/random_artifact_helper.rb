...
module RandomArtifactHelper
  class ArtifactFetcher
    ...
    def self.title(record)
      record["titles"].empty? ? "Unknown" : record["titles"][0]["title"]
    end

    def self.artist_maker(record)
      record["artistMakerPerson"].empty? ? "Unknown" : record["artistMakerPerson"][0]["name"]["text"]
    end

    def self.date(record)
      record["productionDates"].empty? ? "unknown" : record["productionDates"][0]["date"]["text"]
    end
  end
end
